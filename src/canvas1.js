...
const gifURL = "https://66.media.tumblr.com/355746c1617f3a280db88af2b61647bf/tumblr_pshwh7GmH51yph87co1_540.gif";
...
//   }
  ...
  if(myGif) { // If gif object defined
    if(!myGif.loading){  // if loaded
        ...
        // drawParticles();
        drawImage(myGif.image,cw,ch, 1.5,0); 
        image_loaded = true;
    }// displays the current frame.
    // }else if(myGif.lastFrame !== null){  // Shows frames as they load
    //     ctx1.drawImage(myGif.lastFrame.image,0,0); 
    //     ctx1.fillStyle = "white";
    //     ctx1.fillText("GIF loading frame " + myGif.frames.length ,10,21);
    //     ctx1.fillText("GIF loading frame " + myGif.frames.length,10,19);
    //     ctx1.fillText("GIF loading frame " + myGif.frames.length,9,20);
    //     ctx1.fillText("GIF loading frame " + myGif.frames.length,11,20);
    //     ctx1.fillStyle = "black";
    //     ctx1.fillText("GIF loading frame " + myGif.frames.length,10,20);
...
Once loaded the gif can be displayed
    if(!myGif.loading){
        ctx1.drawImage(myGif.image,0,0); 
    }
...
const GIF = function () {
    ...
    function processFrame(frame) { // creates a RGBA canvas image from the indexed pixel data.
        ...
        frame.image.ctx1    = frame.image.getContext("2d");
        ct = frame.localColourTableFlag ? frame.localColourTable : gif.globalColourTable;
        if (gif.lastFrame === null) { gif.lastFrame = frame }
        useT = (gif.lastFrame.disposalMethod === 2 || gif.lastFrame.disposalMethod === 3) ? true : false;
        if (!useT) { frame.image.ctx1.drawImage(gif.lastFrame.image, 0, 0, gif.width, gif.height) }
        cData = frame.image.ctx1.getImageData(frame.leftPos, frame.topPos, frame.width, frame.height);
        ...
        frame.image.ctx1.putImageData(cData, frame.leftPos, frame.topPos);
        gif.lastFrame = frame;
